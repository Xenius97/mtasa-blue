...
// Include appropriate Lua headers based on version detection
    ...
    // Lua 5.1 doesn't have lua_getuservalue/lua_setuservalue
    // Use environment table instead (lua_getfenv/lua_setfenv)
    inline int lua_getuservalue_compat(lua_State* L, int idx, int version) {
        lua_getfenv(L, idx);
        return lua_type(L, -1);
    }
...
inline eLuaVersion GetLuaVersion(lua_State* L) {
    ...
    // Lua 5.1 doesn't have lua_version() function
    // Use LUA_VERSION_NUM at compile time instead
    #if defined(LUA_VERSION_NUM) && LUA_VERSION_NUM == 504
